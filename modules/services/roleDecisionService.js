...
export class RoleDecisionService {
  constructor(dependencies) {
    ...
    this.pendingObservationCommands = null;
  }
  ...
  captureTurnContext(userMessageId) {
    ...
    const current = [...messages]
      .reverse()
      .find(message => Number(message.message_id) === messageId && this._isUserMessage(message));
    if (!current) return null;

    const previousAssistant = [...messages]
      .reverse()
      .find(
        message => Number(message.message_id) < messageId && this._isAssistantMessage(message) && !message.is_hidden,
      );
    const context = {
  ...
  _buildRoleSystemPrompt(commandGuide, outputRules, roleProfiles) {
    ...
    const roleIndex =
      roleNames.length > 0
        ? roleNames
            .map((roleName, index) => `${index + 1}. ${roleName}（来源标签 <${roleName}></${roleName}>）`)
            .join('\n')
        : '未导入角色；不得自行创造需要扮演的人物。';
    const profileText =
      roleProfiles.length > 0 ? roleProfiles.map(profile => profile.content).join('\n\n') : '未导入角色人设。';
    return [
      '你是 SillyView 的幕后角色决策 AI，不是与用户直接对话的前台 AI。',
      '你的任务是分别进入角色索引中人物的身份，依据各自人设进行扮演，并根据上一条角色正文和用户本轮信息判断其内心活动、下一步剧情倾向，以及是否会主动查看手机、市场或自己的账户。若上条对话并未出现你所需要扮演的角色或与你所扮演角色无关，视为角色自由活动，限制所有角色认知范围，确保其仅基于当前时间点已知信息进行反应，禁止出现并未参与正文却能对正文内容进行反应的全知问题，保持真实性',
      ...
      '',
      '示例:- *他看起来不太可靠，我得小心点...*- *这把剑比想象中重，希望不会影响战斗*',
      '内容要求:',
      '特点:- 对角色与NPC和环境的真实感受- 角色当前处境的评估和担忧- 隐藏的动机和计划- 对他人话语的怀疑或赞同- 回忆触发或相关联想',
      '原则:- 结合剧情，故事的前后发展，反映角色真实心理活动 -禁止出现类似正文还在聊天，内心活动却在操作市场或使用命令购入股票的、与正文角色行动、内容货不对板的情况，角色的一切行动的第一原则就是必须依托于正文',
      ...
      profileText,
      '',
      'Staged output rule: when this turn needs an Observe command, the first response must only request the observation and emit the single Observe command. Before the observation result returns, do not output any role inner voice, emotion summary, or final plot tendency. After the observation, produce the final role inner voice and plot tendency once, integrating the original reaction with the observation result. If no Observe command is needed, output the final reaction directly.',
      outputRules,
      '',
      commandGuide,
    ].join('\n');
  ...
  async run(context, options = {}) {
    ...
    try {
      const [promptGuides, roleProfiles] = await Promise.all([this._loadPromptGuides(), this._loadRoleProfiles()]);
      ...
      const deferredCommands = this.pendingObservationCommands;
      this.pendingObservationCommands = null;
      let deferredPrompts = [];
      ...
      for (let round = 1; round <= maxObservationRounds; round++) {
        ...
        if (!session.active) {
          observationRounds.push({
            ...
            active: false,
            rejected: session.rejected || [],
            unknown_account_ids: session.unknown_account_ids || [],
          });
          break;
        }
            ...
            [
              ...
              { role: 'assistant', content: output },
              {
                ...
                content: [
                  ...
                  session.context,
                  '',
                  'The observation is now complete. Only now output the final role inner voice and plot tendency. Integrate the original emotional reaction, role personality, user message, and observation result into one final response; do not discard or overwrite the original reaction merely because market data was observed.',
