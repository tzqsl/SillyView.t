...
角色决策请求当前使用酒馆正在使用的模型，但不读取普通聊天历史或市场后台提示词；独立模型配置将在后续接入。单独开启“自动记录截取调试”不会调用模型，只会在每次用户发送时记录消息编号、上一条正文和当前输入，并实时显示在角色观察调试窗口。
...
- `SillyView_accounts`：多角色协同交易总控附加世界书。默认只启用观察/交易说明；K 线摘要和 `sv_account_state_*` 由观察指令临时启用。`sv_accounts_index` 是账户读取和迁移所需的内部活动索引，始终关闭。
