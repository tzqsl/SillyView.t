...
- 挂单系统：限价单、条件单、移动止损和 OCO 双单；按 K 线内部路径触发，OCO 成交后自动撤销同组另一单。
- 资金系统：初始 10000 信用点、贷款、贷款利息、杠杆资金费率。
- 易上手交易：10% / 25% / 50% / 75% / 全仓快捷金额、做多保护、做空保护、实时风险档位。
- 多角色账户：初始化时扫描角色卡绑定世界书中的开户行格式，为每张角色卡创建独立的 `SillyView_accounts_<角色>_<标识>` 总控世界书，并把多个账号写入其中的独立状态词条。
...
- `市价`：按当前价立即交易。
- `限价`：买单必须低于当前价，卖单必须高于当前价。
- `条件`：买单必须高于当前价，卖单必须低于当前价。
- `OCO`：同时创建一张下轨和一张上轨订单；任一订单成交后，另一张自动撤销。

挂单不会预先冻结现金，而是在触发时执行完整的持仓、余额和手续费校验。校验失败的订单会结束并写入订单历史。当前品种的有效挂单显示在交易页底部，可随时撤销。

移动止损使用百分比距离。多头会随新高上移锚点，空头会随新低下移锚点；价格反向触及动态止损价时自动平仓。它可以在开仓时设置，也可以在资产页调整。
...
- `sv_player_portfolio.pending_orders` / `order_history`：当前挂单和最近50条已成交、已撤销或被拒绝的订单记录。
