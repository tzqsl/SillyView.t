...
- 多角色账户：初始化时扫描角色卡绑定世界书中的开户行格式，自动创建多账户世界书和 `SillyView_fx` 附加世界书。
- 市场推进：AI 导演模式和本地快速模式。
...
1. 创建或更新 `SillyView_fx`，并绑定到角色卡附加世界书。
2. 按开户行账户数量创建多个 `SillyView_account_*` 账号世界书。
3. 在 `SillyView_fx` 写入交易指令、实时账目查询、`sv_kline_context` 和最近十条新闻。
...
AI 可使用 `SillyView_fx` 中写入的多账户交易指令：
...
- `SillyView_fx`：多角色协同交易总控附加世界书，包含指令、账目、K线摘要和新闻。
- `SillyView_account_*`：自动创建的角色专属交易账号世界书。
