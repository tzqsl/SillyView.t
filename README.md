...
3. 在 `SillyView_accounts` 写入交易指令、实时账目查询和唯一一份启用的 `sv_kline_context`。
...
- `sv_kline_context`：紧凑 K 线摘要；主世界书副本关闭，`SillyView_accounts` 中的副本启用。
...
- `SillyView_accounts`：多角色协同交易总控附加世界书。只启用交易说明、账目查询和 K 线摘要；索引、扫描报告和 `sv_account_state_*` 原始状态默认关闭。
